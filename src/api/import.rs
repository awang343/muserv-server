...
/// Picks up any audio files sitting loose in the library root that were
/// never tracked at all (e.g. dropped in by hand), moving them into
/// `.storage` so nothing is left duplicated on disk outside of it.
async fn trigger(
    State(state): State<SharedState>,
    Path(lib_id): Path<i64>,
    ...
    let pool = state.pools.get(&lib_id).expect("library id already validated").clone();
    let bg = state.clone();
    tokio::spawn(async move {
        let result = ingest::import_dir(&pool, &lib, &lib.root(), CopyMode::Move).await;
